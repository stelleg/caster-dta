...
## OVERALL TODOS:
# - Create a cross-attention version of TransformerEncoder for the JointGNN to allow for multiple cross-attention passes
#    - While doing this, make dim_feedforward a parameter rather than just 2*the input dimensionality
# - Improve the molecule representation somehow - it's likely the bottleneck here
# - Add more node features for molecule based on cheminformatics
# - Add type annotations to the model and dataset classes to enable TorchScript
# - Consider just using a preexisting model if the one we're trying to train just doesn't work
# - Add a sequence-based feature extractor for both protein and molecule, then concatenate as node features
...
# TODO: Ablation study with the edge thresholds (and type of thresholding?)
...
# TODO: consider dropout at the input layer as well to prevent overfitting (even if we assume 100% completeness)
# TODO: consider adding option for aggr (sum, mean, etc.) for the GNNs
...
# (TODO: move these to arguments or a config file)
# TODO: consider having different parameter groups, one for each GNN
    ...
    for epoch in range(n_epochs):
        ...
        for protein_g, molecule_g, target in train_dl:
            ...
            if torch.isnan(loss).any():
                print("NaN loss before scaling detected")
                ipdb.set_trace()
...
with torch.no_grad():
    ...
    # TODO: move the below to a separate function(s) for clarity and easier maintenance
    ...
    # TODO: improve the speed of this function because this is very slow
